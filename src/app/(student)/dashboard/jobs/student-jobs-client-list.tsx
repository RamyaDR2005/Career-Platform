  ...
  return (
    <div className="space-y-6">
      <div className="relative">
        ...
        <Input 
          className="pl-10 bg-zinc-900 border-zinc-800 text-zinc-100 placeholder:text-zinc-500 max-w-xl"
      ...
      <div className="grid gap-6">
        {filteredJobs.length === 0 ? (
          ...
          filteredJobs.map(job => {
            ...
            return (
              <Card key={job.id} className={`bg-zinc-900 border-zinc-800 flex flex-col md:flex-row items-start ${isDisabled ? 'opacity-80' : ''}`}>
                <div className="flex-1">
                  <CardHeader>
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <Building2 className="w-4 h-4 text-zinc-400" />
                      <span className="text-sm font-medium text-zinc-400">{job.company.name}</span>
                      {job.status === "CLOSED" && (
                    ...
                    <CardTitle className="text-xl text-zinc-100">{job.title}</CardTitle>
                    <CardDescription className="flex items-center gap-4 text-zinc-400 mt-2 flex-wrap">
                ...
                <div className="p-6 md:pl-0 flex flex-col md:flex-row items-center justify-end gap-3 w-full md:w-auto h-full border-t border-zinc-800 md:border-t-0 md:border-l md:border-zinc-800/50 min-h-[150px]">
