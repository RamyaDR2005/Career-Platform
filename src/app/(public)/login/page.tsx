...
type Step = "EMAIL" | "VERIFY_OTP" | "LOGIN" | "REGISTER" | "FORGOT_PASSWORD";
...
export default function UnifiedAuthPage() {
  ...
  const handleEmailCheck = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.exists) {
        setStep("LOGIN");
        setFormData({ ...formData, role: data.role || "STUDENT" });
      } else {
        // Email doesn't exist, start registration by verifying email
      ...
      <Card className="w-full max-w-md border-0 shadow-[rgba(201,90,46,0.35)_0px_8px_25px_0px]" style={{ backgroundColor: "#fff9f2", color: "#1c211d" }}>
        ...
        {step !== "EMAIL" ? (
          <Button 
            ...
            style={{ color: "#706b63" }}
            onClick={() => { setStep("EMAIL"); setFormData({...formData, password: ""}); }}
          >
        ...
        <CardHeader className="space-y-1 mt-6">
          <CardTitle className="text-3xl font-bold tracking-tight text-center">
            {step === "EMAIL" && "Welcome to CareerAI"}
          ...
          <CardDescription className="text-center" style={{ color: "#706b63" }}>
            ...
            {step === "VERIFY_OTP" && `We've sent a 6-digit code to ${formData.email}`}
            {step === "LOGIN" && `Log in to ${formData.email}`}
            {step === "REGISTER" && `Set up an account for ${formData.email}`}
            {step === "FORGOT_PASSWORD" && `We'll send a reset link to ${formData.email}`}
          </CardDescription>
        </CardHeader>
        
        <CardContent>
          {step === "EMAIL" && (
            <form onSubmit={handleEmailCheck} className="space-y-4 animate-in fade-in zoom-in-95 duration-200">
          ...
          {step === "VERIFY_OTP" && (
            <form onSubmit={handleVerifyOtp} className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="space-y-2">
                <Label htmlFor="otp" style={{ color: "#1c211d" }}>Verification Code</Label>
                <Input
                  id="otp"
                  name="otp"
                  type="text"
            ...
            <form onSubmit={handleLogin} className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button 
                    type="button" 
                    style={{ color: "#c95a2e", textDecoration: "none" }} 
                    className="text-xs hover:underline bg-transparent border-none p-0 cursor-pointer"
          ...
          {step === "REGISTER" && (
            <form onSubmit={handleRegister} className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300 max-h-[60vh] overflow-y-auto px-1 -mx-1">
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="password" style={{ color: "#1c211d" }}>Create Password</Label>
                <Input
                  id="password"
                  name="password"
                  type="password"
          ...
          {step === "FORGOT_PASSWORD" && (
            <form onSubmit={handleForgotPassword} className="space-y-4 animate-in fade-in zoom-in-95 duration-200">
              <div className="space-y-2">
                <Label htmlFor="reset-email" style={{ color: "#1c211d" }}>Confirm Email</Label>
                <Input
                  id="reset-email"
                  name="email"
                  type="email"
